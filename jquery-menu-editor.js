...
function MenuEditor(idSelector, options) {
    ...
    var settings = {
        ...
        listOptions: { 
            ...
            opener: {
                ...
                openerClass: 'btn btn-success btn-sm d-inline-flex align-items-center',
            },
            placeholderCss: {'background-color': 'gray'},
            ignoreClass: 'clickable',
            listsClass: "list-unstyled",
    ...
    $main.on('click', '.btnIn', function (e) {
        ...
        if ($prev.length > 0) {
            ...
            if ($ul.length > 0)
                ...
                var $ul = $('<ul>').addClass('list-unstyled').css('padding-top', '10px');
                $prev.append($ul);
                $ul.append($li);
    ...
    function createMenu(arrayItem, depth) {
        var level = (typeof (depth) === 'undefined') ? 0 : depth;
        var $elem = (level === 0) ? $main : $('<ul>').addClass('list-unstyled').css('padding-top', '10px').data("level", level);
        $.each(arrayItem, function (k, v) {
            ...
            var $li = $('<li>').addClass('list-group-item pe-0');
            $li.data(itemObject);
            var $div = $('<div>').addClass('d-flex justify-content-between align-items-center');
            var $leftDiv = $('<div>').addClass('d-flex align-items-center');
            var $i = $('<i>').addClass(v.icon);
            var $span = $('<span>').addClass('txt ms-2').append(v.text);
            var $divbtn =  TButtonGroup();
            $leftDiv.append($i).append($span);
            $div.append($leftDiv).append($divbtn);
            $li.append($div);
            if (isParent) {
    ...
    this.add = function(){
        ...
        var $span = $('<span>').addClass('txt ms-2').append(data.text);
        var $divbtn =  TButtonGroup();
        leftDiv.append($i).append($span);
        div.append(leftDiv).append($divbtn);
        var $li = $("<li>").data(data);
        $li.addClass('list-group-item pe-0').append(div);
        $main.append($li);
        MenuEditor.updateButtons($main);
        resetForm();
    };
